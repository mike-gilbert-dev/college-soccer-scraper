...
- **Targets / knobs:** `DEFAULT_TARGETS` = MSO + WSO D1. Query overrides: `?days=`, `?ahead=`, `?date=YYYY-MM-DD` (anchors season resolution for backfills), `?sport=&division=`.
- **The window runs both directions.** `?days=` (default 2) covers today + the previous day, for score corrections. `?ahead=` (default 7) covers the next 7 days, and is what keeps *future* schedule data current. Both ends are clamped to the season's `start_date` / `end_date`; `?ahead=0` restores the old backwards-only behaviour. The response echoes the resolved dates as `window`.

  Lookahead is not optional polish. NCAA keeps editing a date after it first appears in the feed — kickoffs move, and whole slates are published days later. Backwards-only meant each date was read exactly once, at 08:00 UTC on game day, and every subsequent change stayed invisible until the game had already been played (observed 2026-08-12: NKU@Austin Peay moved 2:00 PM → 11:00 AM ET and the site served the stale time all morning; 45 of the next day's 60 WSO games were missing entirely). It's also what puts unplayed games in the DB at all, which pick'em and the team schedule pages depend on.
- **A TBD kickoff is stored as `start_time = NULL`, decided by flags — never by the clock.** The feed ships a *placeholder* `startTimeEpoch` (usually midnight or 01:00 ET) for games whose time isn't set, so the timestamp alone can't be trusted; `hasKnownStartTime()` gates on `hasStartTime` / `tba` instead. **Do not filter these by hour** — Hawaii's 7:00 PM local IS 1:00 AM ET, and those contests carry the exact same `startTime: "01:00"` as the junk ones. Only the flags separate them, and a time-of-day heuristic would blank every Hawaii / Alaska / late-Pacific game. (2026-08-12: 168 games sat at midnight ET; nulling them also stopped the live cron waking every minute for ~3h a night across 61 dates, since its gate keys on `start_time`.)

  NULL is the representation the rest of the app already expects: [`game_is_open`](supabase/migrations/20260804000002_picks.sql) coalesces it to midnight ET (locks early — the safe direction), `/scores` orders `nullsFirst: false`, and both `/scores` and the team page render **TBD**. Games where NCAA asserts `hasStartTime: true` are kept as-is even when the time looks impossible (a school entering 1:00 AM for a 1:00 PM game) — the feed gives us nothing to override it with, short of a venue-timezone lookup we don't have.
...
- **Backfills must run sequentially.** Every date's ingest upserts the *same* conference / team / team_season rows, so firing many single-date invocations concurrently deadlocks (observed 2026-08-12: 30-wide fan-out → 72 deadlocks + 20 statement timeouts, and the function still returned HTTP 200 because per-target errors are caught and logged to `scrape_log`). Use one invocation with a wide `?ahead=` and let its internal loop walk the dates — a full 89-date season sweep took 118s with zero errors. **A 200 from this function does not mean the ingest succeeded; check `scrape_log`.**
