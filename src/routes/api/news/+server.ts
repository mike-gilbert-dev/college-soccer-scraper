...
import {
	...
	listPublishedArticlesForPlayer,
	type ArticleSport
...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	// (used by the News tab on team pages); ?player=<internal players.id> does
	// the same for the news feed on a player page; otherwise the site-wide feed.
	...
	const playerId = playerRaw ? parseInt(playerRaw, 10) : NaN;

	const { rows, hasMore } = Number.isFinite(playerId)
		? await listPublishedArticlesForPlayer(locals.supabase, { playerId, offset, limit, sport })
